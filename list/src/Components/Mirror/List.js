...
function createData(name, display_name, is_limited_image, sync_method, url, help_url, size, last_timestamp, status) {
    ...
    if (is_limited_image == '1') {
        Limited = <a class="Attribute Attribute-LightPurple default-cursor TextUppercase" title="只对部分内容进行了同步">limited</a>;
    } else {
        Limited = null;
    }

    if (sync_method != null) {
        Sync = <a class="Attribute Attribute-LightRed default-cursor TextUppercase" title="同步方式">{sync_method}</a>;
    } else {
        Sync = null;
    }
